...
function scrubForProvider(value: unknown, depth = 0): unknown {
  ...
  if (typeof value === 'object' && value) {
    return Object.fromEntries(Object.entries(value as Record<string, unknown>).slice(0, 100).map(([key, item]) => [
      key,
      /password|secret|token|api.?key|credential|private.?key/i.test(key) ? '[redacted]' : scrubForProvider(item, depth + 1),
    ]));
  }
...
function boundedStrings(value: unknown, maxItems: number, maxLength: number): string[] {
  return Array.isArray(value) ? value.filter((item): item is string => typeof item === 'string' && item.trim().length >= 1).slice(0, maxItems).map(item => item.trim().slice(0, maxLength)) : [];
}
...
function normalizeIncompleteBrief(value: unknown, issues: Array<{ path: string; message: string }>): BriefPayload {
  ...
  const confidence = Math.min(70, Math.max(0, rawConfidence >= 0 && rawConfidence <= 1 ? rawConfidence * 100 : rawConfidence));

  const findings = (Array.isArray(source.findings) ? source.findings : []).map(sourceRecord).flatMap(item => {
    ...
    return [{
      ...
      finding: finding.slice(0, 300), impact: impact.slice(0, 300),
      evidenceRefs: boundedStrings(item.evidenceRefs, 4, 300),
    }];
  }).slice(0, 6);
  if (!findings.length) findings.push({
    severity: 'high', domain: 'Evidence completeness',
    finding: 'The provider response did not contain a complete, schema-valid engineering finding.',
    impact: 'The AI output cannot support advancement without direct review of the named reports and workspace evidence.',
    evidenceRefs: [],
  });
  ...
  const metrics = (Array.isArray(source.metrics) ? source.metrics : []).map(sourceRecord).flatMap(item => {
  ...
  const sections = (Array.isArray(source.sections) ? source.sections : []).map(sourceRecord).flatMap(item => {
    if (typeof item.title !== 'string' || typeof item.detail !== 'string' || item.detail.trim().length < 10) return [];
    return [{ title: item.title.slice(0, 120), detail: item.detail.slice(0, 600) }];
  }).slice(0, 4);
  if (sections.length < 2) sections.push({ title: 'Schema assurance', detail: 'The provider response was structurally incomplete. Only schema-valid conclusions were retained; malformed fragments were discarded.' });
  if (sections.length < 2) sections.push({ title: 'Decision boundary', detail: 'Use this brief as advisory triage only. Reconcile every retained claim with primary EDA reports before changing the design or release state.' });

  const tradeoffs = boundedStrings(source.tradeoffs, 5, 300);
  if (!tradeoffs.length) tradeoffs.push('No complete evidence-supported tradeoff statement was returned; compare timing, power, area, congestion, verification effort and schedule before advancing.');
  const stopConditions = boundedStrings(source.stopConditions, 5, 300);
  if (!stopConditions.length) stopConditions.push('Stop advancement while any required signoff scenario, report provenance or configured regression guardrail remains unverified.');
  const actions = boundedStrings(source.actions, 6, 300);
  if (!actions.length) actions.push('Reconcile the retained findings against primary reports, identical run configurations and the active corner set.');
  const gates = boundedStrings(source.humanReviewGates, 6, 300);
  gates.push('An accountable engineer must verify all retained claims because the provider response required deterministic schema completion.');

  return briefSchema.parse({
    headline: boundedString(source.headline, 'Engineering review requires additional evidence', 180),
    executiveSummary: boundedString(source.executiveSummary, 'The provider returned an incomplete structured review. Valid engineering conclusions were retained conservatively, but the result cannot support signoff or design advancement without direct evidence review.', 900),
    risk: sourceRisk, confidence, verdict,
    signoffPosition: boundedString(source.signoffPosition, 'Hold any signoff-affecting decision until an accountable engineer verifies the primary reports, scenario coverage and run comparability.', 1_000),
    evidenceQuality: { grade, score: Math.min(70, Math.max(0, typeof quality.score === 'number' ? quality.score : 35)), rationale: boundedString(quality.rationale, 'Evidence quality is capped because the provider response was structurally incomplete and required deterministic completion.', 400) },
    findings,
    cornerCoverage: { covered: boundedStrings(coverage.covered, 12, 160), missing: boundedStrings(coverage.missing, 12, 160), assessment: boundedString(coverage.assessment, 'Scenario coverage was not returned in a complete structure; verify active PVT, RC, constraint and operating-mode coverage manually.', 500) },
    metrics, sections, tradeoffs,
    recommendedExperiments: boundedStrings(source.recommendedExperiments, 5, 300), stopConditions,
    dataGaps: [...boundedStrings(source.dataGaps, 5, 300), `Provider structure incomplete: ${issues.slice(0, 3).map(issue => issue.path || 'root').join(', ')}`].slice(0, 6),
    actions, evidence: boundedStrings(source.evidence, 12, 300), assumptions: boundedStrings(source.assumptions, 6, 300), humanReviewGates: [...new Set(gates)].slice(0, 6),
  });
}
...
async function requestValidatedBrief(input: {
  ...
  const candidate = await generateJSONCompletion<unknown>(input.prompt, {
    ...
    maxTokens: 4_000,
    reasoningMaxTokens: Number.isFinite(configuredReasoning) && configuredReasoning > 0 ? configuredReasoning : undefined,
  ...
  const issues = first.error.issues.slice(0, 20).map(issue => ({ path: issue.path.join('.'), message: issue.message }));
...
export async function createAiDecisionBrief(input: {
  ...
  const deepReviewModel = process.env.OPENROUTER_CHIP_REVIEW_MODEL ?? process.env.OPENROUTER_MODEL ?? 'anthropic/claude-sonnet-4.5';
  ...
  const playbook = featurePlaybooks[input.feature] ?? `Act as the accountable chip-design review lead. Check domain validity, reproducibility, scenario coverage, cross-stage effects, evidence provenance and concrete exit criteria.`;
  ...
  const reviewRequestJson = promptJson(input.reviewRequest ?? {
  ...
  const specialist = await requestValidatedBrief({
    ...
    model, role: 'specialist', temperature: 0.08, schemaName: 'chip_design_specialist_brief',
  });

  const challenged = twoPass ? await requestValidatedBrief({
    ...
    model: criticModel, role: 'challenger', temperature: 0.05, schemaName: 'chip_design_challenger_brief',
  }) : specialist;
  ...
  return {
    ...
    provider: 'OpenRouter',
    model: twoPass ? (model === criticModel ? `${model} · specialist + challenger` : `${model} → ${criticModel}`) : `${model} · specialist`,
    humanStatus: 'pending',
  };
}
