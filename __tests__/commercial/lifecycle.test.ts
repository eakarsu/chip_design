...
import {
  buildLifecycleChatPrompt,
  buildLifecycleProfile,
  CHIP_DESIGN_LIFECYCLE,
  inferLifecyclePhaseId,
  lifecyclePhaseForFeature,
  ...
  projects: [
    {
      ...
      tenantId: 'tenant-1',
      name: 'Atlas NPU',
      description: 'Edge inference accelerator',
      repositoryUrl: 'https://github.com/example/atlas',
      defaultBranch: 'main',
      topModule: 'atlas_top',
      pdkRef: 'foundry16@4.2',
  ...
  constraints: [
    {
      ...
      projectId: 'project-1',
      name: 'functional',
      sdc: 'create_clock -period 1.0 [get_ports clk]',
      version: 1,
      active: true,
      createdAt: '2026-01-01',
    },
  ...
  rtlImpacts: [
    {
      ...
      baseSha: 'base',
      targetSha: 'abc123',
      changedModules: ['atlas_top'],
      timingDeltaNs: 0.01,
      powerDeltaPct: -5,
      congestionDeltaPct: -1,
      drcDelta: 0,
      affectedPaths: ['cpu/macs'],
...
describe('chip-design lifecycle profile', () => {
  ...
  it('calculates progress from retained workspace evidence', () => {
    ...
    expect(profile?.phases.find((phase) => phase.id === 'technology')?.missingEvidence).toContain(
  ...
  it('places chat and governed reviews into the correct lifecycle phases', () => {
    ...
    expect(lifecyclePhaseForFeature('lifecycle-phase:signoff')).toBe('signoff');
  });

  it('adds an advisory AI gate and human disposition to every lifecycle phase', () => {
    const phaseReview = {
      id: 'review-signoff',
      projectId: 'project-1',
      feature: 'lifecycle-phase:signoff',
      headline: 'Signoff evidence can proceed with bounded conditions',
      verdict: 'proceed-with-conditions',
      risk: 'moderate',
      humanStatus: 'accepted',
      createdAt: '2026-08-09',
    } as DecisionBrief;
    const profile = buildLifecycleProfile({ ...workspace, aiReviews: [phaseReview] }, 'project-1')!;
    const signoff = profile.phases.find((phase) => phase.id === 'signoff')!;
    const verification = profile.phases.find((phase) => phase.id === 'verification')!;
    expect(signoff.aiGate).toMatchObject({
      status: 'accepted',
      reviewId: 'review-signoff',
      verdict: 'proceed-with-conditions',
    });
    expect(verification.aiGate.status).toBe('not-reviewed');
    expect(buildLifecycleChatPrompt(profile, 'signoff')).toContain('AI phase gate: accepted');
  });
});
