...
describe('commercial chip-design workspace', () => {
  ...
  it('calculates PPA regressions and RTL impact risk from measured deltas', async () => {
    ...
    expect((await workspaceBundle(admin)).ppaSnapshots.filter(item => item.commitSha === snapshotInput.commitSha)).toHaveLength(1);
  ...
  it('grounds AI review context by tenant and records an accountable human decision', async () => {
    ...
    const saved = await saveDecisionBrief(admin, {
      projectId, feature: 'ppa', headline: 'Hold the commit pending comparable corner evidence',
      executiveSummary: 'The measured regression is material, but the supplied evidence does not prove that baseline and target runs used equivalent constraints and analysis corners.',
      risk: 'high', confidence: 91, verdict: 'hold', signoffPosition: 'Do not advance this commit until the same MMMC set and tool configuration are rerun and independently reviewed.',
      reviewMode: 'two-pass', promptVersion: 'test-v2', evidenceQuality: { grade: 'B', score: 78, rationale: 'Primary reports are named, but run-equivalence provenance is incomplete.' },
      findings: [{ severity: 'high', domain: 'STA', finding: 'WNS regressed beyond the configured guardrail.', impact: 'The target does not meet the declared timing acceptance threshold.', evidenceRefs: ['runs/test/timing.rpt'] }],
      cornerCoverage: { covered: ['ss_0p72v_125c'], missing: ['ff_0p88v_m40c'], assessment: 'Only one active implementation corner is tied to the submitted record.' },
      metrics: [{ label: 'WNS', value: '-0.124 ns' }], sections: [{ title: 'Comparability', detail: 'Confirm identical tool, PDK, SDC, RC and activity assumptions.' }, { title: 'Attribution', detail: 'Use controlled runs before attributing the regression to one RTL block.' }],
      tradeoffs: ['Extra pipeline stages may recover timing while adding latency and clock power.'], recommendedExperiments: ['Repeat baseline and target across the complete active corner set.'], stopConditions: ['Stop release if any signoff corner remains below the configured WNS threshold.'], dataGaps: ['Tool and constraint digests are absent.'],
      actions: ['Run a controlled, configuration-identical comparison.'], evidence: ['runs/test/timing.rpt'], assumptions: ['Submitted units are nanoseconds.'], humanReviewGates: ['STA owner verifies run equivalence and path-group coverage.'],
      provider: 'OpenRouter', model: 'test-model · specialist + challenger', humanStatus: 'pending',
    }, 'ai-save');
    const decided = await decideAiReview(reviewer, saved.id!, 'accepted', 'I verified the named timing report, active corners, and mandatory rerun gate.', 'ai-decision');
