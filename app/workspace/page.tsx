...
function DetailValues({ data }: { data: Record<string, unknown> }) {
  return <Stack divider={<Divider flexItem />} gap={1}>{Object.entries(data).map(([key, value]) => (
    <Box key={key} sx={{ py: 1 }}>
      ...
      {Array.isArray(value) ? <Stack direction="row" gap={0.5} flexWrap="wrap" sx={{ mt: 0.5 }}>{value.map(item => <Chip key={String(item)} size="small" label={String(item)} />)}</Stack>
