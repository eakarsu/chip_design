...
const supported = ['spice-regression', 'co-design', 'library-marketplace', ...PLATFORM_CAPABILITY_IDS] as const;
const schema = z
  .object({
    ...
    payload: z.record(z.unknown()).refine((value) => JSON.stringify(value).length <= 50_000, 'Payload exceeds 50 KB'),
