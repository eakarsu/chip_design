...
// Gap-nonai endpoint: Team collaboration with role-based design ownership
...
export async function POST(req: NextRequest) {
  try {
    if (!process.env.OPENROUTER_API_KEY) return NextResponse.json({ error: 'OPENROUTER_API_KEY missing' }, { status: 503 });
    ...
    const r = await fetch(OPENROUTER_URL, {
      ...
      body: JSON.stringify({ model: OPENROUTER_MODEL, messages: [
        ...
        { role: 'user', content: `Feature: Team collaboration with role-based design ownership\nContext: ${JSON.stringify(body)}\nReturn JSON {"summary":"","key_points":[""],"recommendations":[""]}` }
    ...
    await persist('team-collaboration-with-role-based-design-ownership', body, out);
    return NextResponse.json({ feature: 'team-collaboration-with-role-based-design-ownership', title: 'Team collaboration with role-based design ownership', result: out });
  } catch (err: any) { return NextResponse.json({ error: err?.message || 'error' }, { status: 500 }); }
}
