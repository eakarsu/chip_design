...
function GovernedAIChatContent() {
  ...
  useEffect(() => {
    ...
    void fetch('/api/workspace/bootstrap')
      ...
      .then((data) => {
        if (active) setWorkspace(data.workspace);
      })
      .catch(() => {
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" gap={2} sx={{ mb: 3 }}>
        <Box>
          ...
          <Typography color="text.secondary" sx={{ mt: 1, maxWidth: 820 }}>
            Ask any question about the app, its tools, your design workflow, or chip-design concepts. Your conversation
            stays with you when you navigate to another page and open Ask AI.
          </Typography>
        </Box>
        <Stack direction="row" gap={1} alignItems="flex-start" flexWrap="wrap" useFlexGap>
          <Button
            component={Link}
            href={`/governed-ai/lifecycle${projectId ? `?projectId=${encodeURIComponent(projectId)}` : ''}${phaseId ? `#phase-${encodeURIComponent(phaseId)}` : ''}`}
            startIcon={<ArrowBack />}
            variant="outlined"
          >
            Back to lifecycle
          ...
          {phaseId && <Chip icon={<Gavel />} label="Human signoff required" variant="outlined" />}
        </Stack>
      </Stack>
      {phaseId && (
        <Alert severity="info" sx={{ mb: 2 }}>
          AI guidance is advisory. Verify claims against retained design reports, approved constraints and signoff
          evidence before making a release decision.
        </Alert>
      )}
      {projectId && phaseId && !workspace && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Loading the selected project and lifecycle evidence…
        </Alert>
      )}
      {profile && phase && (
        <Alert
          ...
          action={
            <Button
              component={Link}
              href={`/governed-ai/lifecycle?projectId=${encodeURIComponent(profile.project.id)}`}
              color="inherit"
            >
              Open lifecycle
            </Button>
        ...
        >
          Context loaded: <strong>{profile.project.name}</strong> · Phase {phase.order}: {phase.title} ·{' '}
          {phase.progress}% evidence complete. The review prompt below contains retained and missing evidence.
        </Alert>
      )}
      <AICopilot
        embedded
        title={phase ? `${phase.order}. ${phase.title}` : 'Your conversation'}
        initialMode={phaseId ? 'review' : undefined}
        ...
        lifecycleContext={
          profile
            ? {
                ...
                phases: profile.phases.map((item) => ({
                  id: item.id,
                  order: item.order,
                  title: item.title,
                  status: item.status,
