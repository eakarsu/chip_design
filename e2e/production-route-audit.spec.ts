...
test('every application page renders without a server or browser exception', async ({ page, baseURL }) => {
  ...
  let designs = await page.request.get(`${baseURL}/api/openlane/designs`).then(response => response.json()).catch(() => ({ designs: [] }));
  let runs = await page.request.get(`${baseURL}/api/openlane/runs`).then(response => response.json()).catch(() => ({ runs: [] }));
  let auditDesignId: string | undefined;
  if (!designs.designs?.length) {
    const created = await page.request.post(`${baseURL}/api/openlane/designs`, {
      data: {
        ...
        ports: [{ name: 'clk', direction: 'input' }, { name: 'd', direction: 'input' }, { name: 'q', direction: 'output' }],
  ...
  if (!runs.runs?.length && designs.designs?.[0]?.id) {
    const created = await page.request.post(`${baseURL}/api/openlane/runs`, {
  ...
  for (const route of pageRoutes) {
    const resolvedRoute = route
      ...
      .replace('/openlane/designs/00000000-0000-4000-8000-000000000001', `/openlane/designs/${designs.designs?.[0]?.id ?? 'missing'}`)
      .replace('/openlane/runs/00000000-0000-4000-8000-000000000001', `/openlane/runs/${runs.runs?.[0]?.id ?? 'missing'}`);
